...
pub fn get_yy_thunks_obj_path(os: OS, arch: Arch) -> Option<PathBuf> {
    match (os, arch) {
        ...
        (OS::Windows10, Arch::Win32) => {
            Some(PathBuf::from("objs/x86/YY_Thunks_for_Win10.0.10240.obj"))
        }
        (OS::Windows10, Arch::X64) => {
            Some(PathBuf::from("objs/x64/YY_Thunks_for_Win10.0.10240.obj"))
        }
        (OS::Windows10_20H1, Arch::Win32) => {
            Some(PathBuf::from("objs/x86/YY_Thunks_for_Win10.0.19041.obj"))
        }
        (OS::Windows10_20H1, Arch::X64) => {
            Some(PathBuf::from("objs/x64/YY_Thunks_for_Win10.0.19041.obj"))
        }
        _ => None,
    }
}
