...
impl Connection {
    ...
    /// Transmit one bounded batch of unreliable application datagrams.
    ///
    /// This method locks the connection once for each ready queue state. It
    /// wakes the driver once after it queues one or more datagrams. It waits
    /// for capacity without discarding older datagrams.
    pub fn send_datagrams_wait(&self, data: VecDeque<Bytes>) -> SendDatagrams<'_> {
        SendDatagrams {
            conn: &self.0,
            data,
            notify: self.0.shared.datagrams_unblocked.notified(),
...
pin_project! {
    /// Future produced by [`Connection::send_datagrams_wait`].
    pub struct SendDatagrams<'a> {
        conn: &'a ConnectionRef,
        data: VecDeque<Bytes>,
        #[pin]
        notify: Notified<'a>,
...
impl Future for SendDatagrams<'_> {
    type Output = (Result<(), SendDatagramError>, VecDeque<Bytes>);

    fn poll(self: Pin<&mut Self>, ctx: &mut Context<'_>) -> Poll<Self::Output> {
        let mut this = self.project();
        let mut state = this.conn.state.lock("SendDatagrams::poll");
        if let Some(ref error) = state.error {
            let data = std::mem::take(this.data);
            return Poll::Ready((Err(SendDatagramError::ConnectionLost(error.clone())), data));
        }

        let mut queued = false;
        while let Some(data) = this.data.pop_front() {
            use proto::SendDatagramError::*;
            match state.inner.datagrams().send(data, false) {
                Ok(()) => queued = true,
                Err(Blocked(data)) => {
                ...
                Err(error) => {
                    ...
                    let error = match error {
                        ...
                        Disabled => SendDatagramError::Disabled,
                        TooLarge => SendDatagramError::TooLarge,
                        Blocked(_) => unreachable!(),
                    };
                    let data = std::mem::take(this.data);
                    return Poll::Ready((Err(error), data));
                }
            }
        }
        ...
        if this.data.is_empty() {
            return Poll::Ready((Ok(()), std::mem::take(this.data)));
        }
        drop(state);
