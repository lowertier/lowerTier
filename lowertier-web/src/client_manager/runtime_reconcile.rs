...
fn normalized_port_forwards(
    ...
    Ok(config
        ...
        .into_iter()
        .map(|cfg| {
            RuntimePortForwardConfig::from(lowertier::proto::common::PortForwardConfigPb::from(cfg))
        })
        .collect())
}
...
mod tests {
    ...
    fn runtime_patch_rejects_credential_private_key_change() {
        ...
        current.secure_mode = Some(lowertier::proto::common::SecureModeConfig {
            enabled: true,
            local_private_key: Some("mUuD5fsIm/ftvgS4WBAYFMNLqWX3qT9rnm4PrnOqb9s=".to_string()),
            local_public_key: None,
        });
        let mut desired = current.clone();
        desired.secure_mode = Some(lowertier::proto::common::SecureModeConfig {
            enabled: true,
    ...
    fn runtime_patch_rejects_explicit_desired_hostname_change() {
        ...
        let mut desired =
            config_with_port_forwards(vec![port_forward(23000, 5174), port_forward(23007, 3389)]);
        desired.hostname =
            Some(lowertier::common::config::TomlConfigLoader::default().get_hostname());
