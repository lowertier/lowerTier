...
import { I18nUtils, RemoteManagement, Utils } from "lowertier-frontend-lib"
...
import { getLowTierVersion, getServiceStatus } from '~/composables/backend'
...
async function initWithMode(mode: Mode) {
  ...
  switch (mode.mode) {
    ...
    case 'service': {
      ...
      let serviceStatus = await getServiceStatus()
      const coreVersion = await getLowTierVersion()
...
onMounted(async () => {
  ...
  if (type() === 'android') {
    try {
      ...
      console.error("lowertier init vpn service done")
    } catch (e: any) {
      console.error("lowertier init vpn service failed", e)
    }
  }
...
async function connectRpcClient(isNormalMode: boolean, url?: string) {
  ...
  console.log("lowertier rpc connection established, isNormalMode: ", isNormalMode)
}
