...
#[tauri::command]
fn lowertier_version() -> Result<String, String> {
    Ok(lowertier::VERSION.to_string())
}
...
fn get_service_status() -> Result<&'static str, String> {
    #[cfg(not(target_os = "android"))]
    {
        use lowertier::service_manager::ServiceStatus;
...
async fn init_web_client(app: AppHandle, url: Option<String>) -> Result<(), String> {
    ...
    let web_client = web_client::run_web_client(
        url.as_str(),
        lowertier::common::MachineIdOptions {
...
mod manager {
    ...
    use lowertier::common::global_ctx::GlobalCtx;
    use lowertier::common::stun::MockStunInfoCollector;
    use lowertier::launcher::NetworkConfig;
    use lowertier::proto::api::logger::{
        LoggerRpc, LoggerRpcClientFactory, SetLoggerConfigRequest,
    };
    use lowertier::proto::api::manage::RunNetworkInstanceRequest;
    use lowertier::proto::common::NatType;
    use lowertier::proto::rpc_impl::bidirect::BidirectRpcManager;
    use lowertier::proto::rpc_types::controller::BaseController;
    use lowertier::rpc_service::logger::LoggerRpcService;
    use lowertier::rpc_service::remote_client::PersistentConfig;
    use lowertier::tunnel::TunnelConnector;
    use lowertier::tunnel::ring::RingTunnelConnector;
    use lowertier::web_client::WebClientHooks;
    ...
    impl WebClientHooks for GuiHooks {
        async fn pre_run_network_instance(
            &self,
            cfg: &lowertier::common::config::TomlConfigLoader,
        ) -> Result<(), String> {
            let client_manager = get_client_manager!()?;
            client_manager
                .pre_run_network_instance_hook(
    ...
    impl GUIClientManager {
        pub async fn new(rpc_url: Option<String>) -> Result<Self, anyhow::Error> {
            ...
            let tunnel = if let Some(url) = rpc_url {
                let mut connector = lowertier::connector::create_connector_by_url(
                    &url,
                    &global_ctx,
                    lowertier::tunnel::IpVersion::Both,
                )
                .await?;
        ...
        pub(super) async fn disable_instances_with_tun(
            ...
            app: &AppHandle,
            web_only: bool,
        ) -> Result<(), lowertier::rpc_service::remote_client::RemoteClientError<anyhow::Error>>
        {
                        ...
                        tokio::spawn(async move {
                            ...
                            loop {
                                match event_receiver.recv().await {
                                    Ok(lowertier::common::global_ctx::GlobalCtxEvent::DhcpIpv4Changed(_, _)) => {
                                        let _ = app_clone.emit("dhcp_ip_changed", &instance_id_str);
                                    }
                                    Ok(lowertier::common::global_ctx::GlobalCtxEvent::ProxyCidrsUpdated(_, _)) => {
...
mod service {
    ...
    pub fn install(opts: ServiceOptions) -> anyhow::Result<()> {
        let service = lowertier::service_manager::Service::new(env!("CARGO_PKG_NAME").to_string())?;
        let options = lowertier::service_manager::ServiceInstallOptions {
            program: super::get_exe_path().into(),
            args: opts.to_args_vec(),
            work_directory: std::env::current_dir()?,
            disable_autostart: false,
            description: Some("LowTier Gui Service".to_string()),
            display_name: Some("LowTier Gui Service".to_string()),
    ...
    pub fn status() -> anyhow::Result<lowertier::service_manager::ServiceStatus> {
        let service = lowertier::service_manager::Service::new(env!("CARGO_PKG_NAME").to_string())?;
        service.status()
    }
}
...
pub fn run_cli() -> std::process::ExitCode {
    tokio::runtime::Builder::new_current_thread()
        ...
        .build()
        .unwrap()
        .block_on(async { lowertier::core::main().await })
}
