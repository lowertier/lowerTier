# LowTier Uptime Monitor

一个用于监控 LowTier 实例健康状态和运行时间的系统。

## 功能特性

- 🏥 **健康监控**: 实时监控 LowTier 节点的健康状态
- 📊 **数据统计**: 提供详细的运行时间和响应时间统计
- 🔧 **实例管理**: 管理多个 LowTier 实例
- 🌐 **Web界面**: 直观的 Web 管理界面
- 🚨 **告警系统**: 支持健康状态异常告警
- 📈 **图表展示**: 可视化展示监控数据
...
| 变量名 | 默认值 | 说明 |
|--------|--------|------|
| `VITE_APP_TITLE` | `LowTier Uptime Monitor` | 应用标题 |
...
```bash
# 构建镜像
docker build -t lowertier-uptime .

# 运行容器
docker run -d -p 8080:8080 lowertier-uptime
```
