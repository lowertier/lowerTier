...
static void print_usage(void) {
    printf("Set LOWTIER_FFI_CONFIG and LOWTIER_FFI_INSTANCE to run the async data-plane demo.\n");
    printf("Optional demos:\n");
    printf("  LOWTIER_FFI_TARGET=10.0.0.2:22          async TCP connect/read\n");
    printf("  LOWTIER_FFI_LISTEN_PORT=12345           async TCP bind/accept/read/write\n");
    printf("  LOWTIER_FFI_UDP_TARGET=10.0.0.2:9000    async UDP bind/send_to/recv_from\n");
}

int main(void) {
    const char *config = getenv("LOWTIER_FFI_CONFIG");
    const char *instance = getenv("LOWTIER_FFI_INSTANCE");
    if (!config || !instance) {
        print_usage();
        return 0;
    }
    ...
    int failed = 0;
    const char *target = getenv("LOWTIER_FFI_TARGET");
    if (target) {
        failed |= run_tcp_connect_demo(instance, target) != 0;
    }

    const char *listen_port = getenv("LOWTIER_FFI_LISTEN_PORT");
    if (listen_port) {
        failed |= run_tcp_listen_demo(instance, listen_port) != 0;
    }

    const char *udp_target = getenv("LOWTIER_FFI_UDP_TARGET");
    if (udp_target) {
