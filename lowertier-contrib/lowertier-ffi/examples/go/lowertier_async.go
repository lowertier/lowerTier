// Package lowertierffi contains a small Go wrapper around the LowTier FFI
// examples. This file documents the async data-plane surface; the synchronous
// wrapper lives in lowertier.go.
//
// Public async entry points:
//
//   - OpenAsync(path) loads the LowTier FFI dynamic library and binds the
...
//     LowTier state.
//
//   - (*AsyncNative).RunNetworkInstance(config) starts one LowTier instance
