...
func (n *Native) CallJSONRPC(serviceName, methodName, domainName, payloadJSON string) (string, error) {
	...
	if response == nil {
		return "", errors.New("lowertier ffi JSON RPC returned nil response")
	}
...
func (n *Native) lastError() error {
	...
	if out == nil {
		return errors.New("lowertier ffi call failed")
	}
	msg := readCString(out)
	_ = n.freeCString(out)
...
func opError(op string, addr net.Addr, err error) error {
	return &net.OpError{Op: op, Net: "lowertier", Addr: addr, Err: err}
}
...
func parseIPPort(address string) (net.IP, int, error) {
	...
	ip := net.ParseIP(host)
	if ip == nil {
		return nil, 0, fmt.Errorf("lowertier ffi requires an IP address, got %q", host)
	}
...
func parseListenPort(address string) (int, error) {
	...
	if host != "" {
		ip := net.ParseIP(host)
		if ip == nil {
			return 0, fmt.Errorf("lowertier ffi requires an IP address, got %q", host)
		}
		if !ip.IsUnspecified() {
			return 0, fmt.Errorf("lowertier ffi listen address must be unspecified, got %q", host)
		}
	}
	port, err := strconv.ParseUint(portStr, 10, 16)
	if err != nil {
...
func cString(s string) []byte {
	if strings.ContainsRune(s, 0) {
		panic("lowertier ffi string contains NUL")
	}
...
func defaultLibraryPath() string {
	if p := os.Getenv("LOWTIER_FFI_LIB"); p != "" {
		return p
	}
	switch runtime.GOOS {
	case "darwin":
		return "../../../../target/debug/liblowertier_ffi.dylib"
	case "windows":
		return "..\\..\\..\\..\\target\\debug\\lowertier_ffi.dll"
	default:
		return "../../../../target/debug/liblowertier_ffi.so"
	}
}
