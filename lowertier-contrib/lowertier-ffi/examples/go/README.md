...
This demo wraps LowTier FFI data-plane TCP as Go `net.Conn` and `net.Listener`.
It can connect to an SSH server through LowTier and read its banner, or accept a
TCP connection from another LowTier peer and run a small ping/pong exchange.
The async op-handle wrapper is in `lowertier_async.go`; the original synchronous
wrapper stays in `lowertier.go`.
...
```sh
cargo build -p lowertier-ffi --features ffi-dataplane
```

The demo loads the debug library by default:

```text
target/debug/liblowertier_ffi.so
```

To use another library path, export `LOWTIER_FFI_LIB=/path/to/liblowertier_ffi.so`.

## 1.2. Configure the LowTier config

`LOWTIER_FFI_CONFIG` is a string of the LowTier config in TOML format which is passed to the FFI library. For example:

```sh
export LOWTIER_FFI_CONFIG='instance_name = "default"
ipv4 = "10.0.0.1"
...
Set the local instance name and a SSH server target to connect through LowTier:

```sh
export LOWTIER_FFI_INSTANCE=default
export LOWTIER_FFI_TARGET=10.0.0.2:22
```
...
```sh
export LOWTIER_FFI_LISTEN_CONFIG='instance_name = "listener"
ipv4 = "10.0.0.3"
...
uri = "tcp://123.123.123.123:11010"
'
export LOWTIER_FFI_LISTEN_INSTANCE=listener
export LOWTIER_FFI_LISTEN_PORT=12345
```
...
```sh
cd lowertier-contrib/lowertier-ffi/examples/go
CGO_ENABLED=0 go test -v ./...
```

The synchronous tests use the environment variables above. The async Go tests
are self-contained: they start two local LowTier instances in the same test
...
payload)` for non-lifecycle LowTier RPCs. For example,
...
For `TestTCPListenIntegration`, connect from another LowTier peer to the local
LowTier IPv4 address and `LOWTIER_FFI_LISTEN_PORT`, send `ping`, and expect
`pong` in response.
...
cc -Wall -Wextra -pedantic \
  ...
  -L ../../../../target/debug -llowertier_ffi \
  -Wl,-rpath,../../../../target/debug \
  -o /tmp/lowertier_data_plane_async

/tmp/lowertier_data_plane_async
```

Without environment variables it prints usage and exits successfully. With
`LOWTIER_FFI_CONFIG`, `LOWTIER_FFI_INSTANCE`, and one of
`LOWTIER_FFI_TARGET`, `LOWTIER_FFI_LISTEN_PORT`, or `LOWTIER_FFI_UDP_TARGET`,
