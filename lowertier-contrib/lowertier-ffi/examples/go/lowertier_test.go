...
func TestSSHIntegration(t *testing.T) {
	config := os.Getenv("LOWTIER_FFI_CONFIG")
	instance := os.Getenv("LOWTIER_FFI_INSTANCE")
	target := os.Getenv("LOWTIER_FFI_TARGET")
	if config == "" || instance == "" || target == "" {
		t.Skip("set LOWTIER_FFI_CONFIG, LOWTIER_FFI_INSTANCE and LOWTIER_FFI_TARGET to run integration test")
	}
...
func TestTCPListenIntegration(t *testing.T) {
	config := os.Getenv("LOWTIER_FFI_LISTEN_CONFIG")
	instance := os.Getenv("LOWTIER_FFI_LISTEN_INSTANCE")
	listenPort := os.Getenv("LOWTIER_FFI_LISTEN_PORT")
	if config == "" || instance == "" || listenPort == "" {
		t.Skip("set LOWTIER_FFI_LISTEN_CONFIG, LOWTIER_FFI_LISTEN_INSTANCE and LOWTIER_FFI_LISTEN_PORT to run integration test")
	}
	...
	t.Logf("listening on %s; connect from another LowTier peer and send ping", listener.Addr())
