...
/**
 * LowTier data-plane API for Android.
 *
 * Dataplane APIs do not create or start an LowTier instance by themselves.
 * Start an instance with [LowTierJNI.runNetworkInstance] first, then pass the
 * same `instanceName` to [LowTierDataPlane.tcpConnect],
 * [LowTierDataPlane.tcpBind], or [LowTierDataPlane.udpBind]. If that instance
 * is not running, the native start call fails and the coroutine wrapper throws
 * the last LowTier FFI error.
 *
 * Typical setup:
 * ```
 ...
 * """.trimIndent()
 *
 * LowTierJNI.runNetworkInstance(config)
 * ```
 *
 * After the instance is running, most callers should use [LowTierDataPlane]
 * and the socket/stream classes below. [LowTierDataPlaneJNI] is the low-level
 * native op-handle ABI used by the coroutine wrappers.
 *
 * TCP client usage:
 * ```
 * val stream = LowTierDataPlane.tcpConnect(instanceName, "10.144.0.2", 8080, 5_000)
 * try {
 ...
 * TCP server usage:
 * ```
 * val listener = LowTierDataPlane.tcpBind(instanceName, 8080, 5_000)
 * try {
 ...
 * UDP usage:
 * ```
 * val socket = LowTierDataPlane.udpBind(instanceName, 0, 5_000)
 * try {
...
/** Data-plane IPv4/port pair returned by LowTier FFI. */
...
class DataPlaneTcpStream(
    ...
    suspend fun read(maxLength: Int, timeoutMs: Long): ByteArray =
            LowTierDataPlane.tcpRead(this, maxLength, timeoutMs)

    /** Write [data], waiting at most [timeoutMs] in native code. */
    suspend fun write(data: ByteArray, timeoutMs: Long): Int =
            LowTierDataPlane.tcpWrite(this, data, timeoutMs)

    /** Close the native TCP stream handle. */
    fun close(): Int = LowTierDataPlaneJNI.dataPlaneTcpClose(handle)
}
...
class DataPlaneTcpListener(val handle: Long, val localAddress: DataPlaneSocketAddress) {
    ...
    suspend fun accept(timeoutMs: Long): DataPlaneTcpStream =
            LowTierDataPlane.tcpAccept(this, timeoutMs)

    /** Close the native TCP listener handle. */
    fun close(): Int = LowTierDataPlaneJNI.dataPlaneTcpListenerClose(handle)
}
...
class DataPlaneUdpSocket(val handle: Long, val localAddress: DataPlaneSocketAddress) {
    ...
    suspend fun sendTo(
            ...
            data: ByteArray,
            timeoutMs: Long
    ): Int = LowTierDataPlane.udpSendTo(this, dstIp, dstPort, data, timeoutMs)

    /** Receive one UDP datagram and its peer address. */
    suspend fun recvFrom(maxLength: Int, timeoutMs: Long): DataPlaneUdpRecvResult =
            LowTierDataPlane.udpRecvFrom(this, maxLength, timeoutMs)

    /** Close the native UDP socket handle. */
    fun close(): Int = LowTierDataPlaneJNI.dataPlaneUdpClose(handle)
}
 ...
 * for completeness, but most Android callers should use [LowTierDataPlane]
 * instead so coroutine cancellation and op cleanup are handled consistently.
 */
object LowTierDataPlaneJNI {
    init {
        System.loadLibrary("lowertier_android_jni")
    }
...
object LowTierDataPlane {
    ...
    /** Connect to a TCP endpoint through the named LowTier instance. */
    @JvmStatic
    suspend fun tcpConnect(
        ...
        val op =
                requireOp(
                        LowTierDataPlaneJNI.dataPlaneTcpConnectStart(
                                instanceName,
                                dstIp,
                                dstPort,
                                timeoutMs
        ...
        val result = awaitOp(op) {
            LowTierDataPlaneJNI.dataPlaneTcpConnectFinish(it) ?: throw lastDataPlaneException()
        }
        return DataPlaneTcpStream(result.handle, result.localAddress)
    }

    /** Bind a TCP data-plane listener on [localPort]. Port 0 asks LowTier to allocate one. */
    @JvmStatic
    suspend fun tcpBind(
            instanceName: String,
            localPort: Int,
        ...
        val op =
                requireOp(
                        LowTierDataPlaneJNI.dataPlaneTcpBindStart(
                                instanceName,
                                localPort,
                                timeoutMs
                        )
        ...
        val result = awaitOp(op) {
            LowTierDataPlaneJNI.dataPlaneTcpBindFinish(it) ?: throw lastDataPlaneException()
        }
    ...
    suspend fun tcpAccept(listener: DataPlaneTcpListener, timeoutMs: Long): DataPlaneTcpStream {
        val op =
                requireOp(
                        LowTierDataPlaneJNI.dataPlaneTcpAcceptStart(listener.handle, timeoutMs)
                )
        val result = awaitOp(op) {
            LowTierDataPlaneJNI.dataPlaneTcpAcceptFinish(it) ?: throw lastDataPlaneException()
        }
    ...
    suspend fun tcpRead(
        ...
        val op =
                requireOp(
                        LowTierDataPlaneJNI.dataPlaneTcpReadStart(
                                stream.handle,
                                maxLength,
                                timeoutMs
                        )
        ...
        return awaitOp(op) {
            LowTierDataPlaneJNI.dataPlaneTcpReadFinish(it)?.data
    ...
    suspend fun tcpWrite(stream: DataPlaneTcpStream, data: ByteArray, timeoutMs: Long): Int {
        val op =
                requireOp(
                        LowTierDataPlaneJNI.dataPlaneTcpWriteStart(
                                stream.handle,
                                data,
                                timeoutMs
                        )
                )
        return awaitOp(op) { LowTierDataPlaneJNI.dataPlaneTcpWriteFinish(it) }
    }

    /** Bind a UDP data-plane socket on [localPort]. Port 0 asks LowTier to allocate one. */
    @JvmStatic
    suspend fun udpBind(
            instanceName: String,
            localPort: Int,
        ...
        val op =
                requireOp(
                        LowTierDataPlaneJNI.dataPlaneUdpBindStart(
                                instanceName,
                                localPort,
                                timeoutMs
                        )
        ...
        val result = awaitOp(op) {
            LowTierDataPlaneJNI.dataPlaneUdpBindFinish(it) ?: throw lastDataPlaneException()
        }
    ...
    suspend fun udpSendTo(
        ...
        val op =
                requireOp(
                        LowTierDataPlaneJNI.dataPlaneUdpSendToStart(
                                socket.handle,
                                dstIp,
                                dstPort,
                                data,
        ...
        return awaitOp(op) { LowTierDataPlaneJNI.dataPlaneUdpSendToFinish(it) }
    }
    ...
    suspend fun udpRecvFrom(
        ...
        val op =
                requireOp(
                        LowTierDataPlaneJNI.dataPlaneUdpRecvFromStart(
                                socket.handle,
                                maxLength,
                                timeoutMs
                        )
        ...
        return awaitOp(op) {
            LowTierDataPlaneJNI.dataPlaneUdpRecvFromFinish(it) ?: throw lastDataPlaneException()
        }
    }
    ...
    private suspend fun <T> awaitOp(op: Long, finish: (Long) -> T): T =
            withContext(Dispatchers.IO) {
                ...
                try {
                    ...
                    LowTierDataPlaneJNI.dataPlaneAsyncOpCancel(op)
                    throw e
                } finally {
                    if (!consumed) {
                        LowTierDataPlaneJNI.dataPlaneAsyncOpFree(op)
                    }
                }
            }
    ...
    private suspend fun awaitReady(op: Long) {
        while (true) {
            ...
            when (LowTierDataPlaneJNI.dataPlaneAsyncOpWait(op, DATA_PLANE_WAIT_SLICE_MS)) {
    ...
    private fun lastDataPlaneException(): RuntimeException {
        return RuntimeException(LowTierJNI.getLastError() ?: "LowTier data-plane call failed")
    }
}
