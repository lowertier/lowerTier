...
/** LowTier 管理类 负责管理 LowTier 实例的生命周期、监控网络状态变化、控制 VpnService */
class LowTierManager(
        private val activity: Activity,
        private val instanceName: String,
        private val networkConfig: String
) {
    companion object {
        private const val TAG = "LowTierManager"
    ...
    fun start() {
        if (isRunning) {
            Log.w(TAG, "LowTier 实例已经在运行中")
            return
        }

        try {
            // 启动 LowTier 实例
            val result = LowTierJNI.runNetworkInstance(networkConfig)
            if (result == 0) {
                isRunning = true
                Log.i(TAG, "LowTier 实例启动成功: $instanceName")

                // 开始监控网络状态
                handler.post(monitorRunnable)
            } else {
                Log.e(TAG, "LowTier 实例启动失败: $result")
                val error = LowTierJNI.getLastError()
                Log.e(TAG, "错误信息: $error")
            }
        } catch (e: Exception) {
            Log.e(TAG, "启动 LowTier 实例时发生异常", e)
        }
    }
    ...
    fun stop() {
        ...
        try {
            ...
            // 停止 LowTier 实例
            LowTierJNI.stopAllInstances()
            Log.i(TAG, "LowTier 实例已停止: $instanceName")

            // 重置状态
            currentIpv4 = null
            currentProxyCidrs = emptyList()
        } catch (e: Exception) {
            Log.e(TAG, "停止 LowTier 实例时发生异常", e)
        }
    }
    ...
    private fun monitorNetworkStatus() {
        try {
            val infosJson = LowTierJNI.collectNetworkInfos(10)
            ...
            if (!networkInfo.running) {
                Log.w(TAG, "LowTier 实例未运行: ${networkInfo.error_msg}")
                return
            }
            ...
            if (newIpv4Inet == null) {
                Log.w(TAG, "LowTier No Ipv4: $networkInfo")
                return
            }
    ...
    private fun startVpnService(ipv4: String, proxyCidrs: List<String>) {
        try {
            val intent = Intent(activity, LowTierVpnService::class.java)
    ...
    /** 获取当前状态信息 */
    fun getStatus(): LowTierStatus {
        return LowTierStatus(
                isRunning = isRunning,
