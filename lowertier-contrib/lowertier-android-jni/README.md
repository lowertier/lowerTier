...
这是 LowTier 的 Android JNI 绑定库，允许 Android 应用程序调用 LowTier 的网络功能。

## 功能特性

- 🚀 完整的 LowTier FFI 接口封装
- 📱 原生 Android JNI 支持
- 🔧 支持多种 Android 架构 (arm64-v8a, armeabi-v7a, x86, x86_64)
- 🛡️ 类型安全的 Java 接口
- 🔌 支持通过 JSON 调用已暴露的 LowTier RPC 查询/管理接口
- 📝 详细的错误处理和日志记录
...
1. **克隆项目并进入目录**
   ```bash
   cd /path/to/LowTier/lowertier-contrib/lowertier-android-jni
   ```
...
将 `java/com/lowertier/jni/LowTierJNI.java` 复制到您的 Android 项目的相应包路径下。
...
```java
import com.lowertier.jni.LowTierJNI;
import java.util.Map;

public class LowTierManager {
    ...
    public void startNetwork() {
        ...
        try {
            // 解析配置
            int result = LowTierJNI.parseConfig(config);
            if (result != 0) {
                String error = LowTierJNI.getLastError();
                throw new RuntimeException("配置解析失败: " + error);
            }
            
            // 启动网络实例
            result = LowTierJNI.runNetworkInstance(config);
            if (result != 0) {
                String error = LowTierJNI.getLastError();
                throw new RuntimeException("网络实例启动失败: " + error);
            }
            
            System.out.println("LowTier 网络实例启动成功");
    ...
    public void getNetworkInfo() {
        try {
            Map<String, String> infos = LowTierJNI.collectNetworkInfosAsMap(10);
    ...
    public void stopNetwork() {
        try {
            int result = LowTierJNI.stopAllInstances();
            if (result == 0) {
...
`LowTierJNI.callJsonRpc(serviceName, methodName, domainName, payloadJson)` 可以调用已暴露的
LowTier RPC 服务，payload 和返回值均为 protobuf JSON。该接口不支持
`api.manage.WebClientService`；实例启动、保留、删除、信息收集仍使用专用 JNI API。

```java
String response = LowTierJNI.callJsonRpc(
...
```java
public class LowTierVpnService extends VpnService {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (vpnInterface != null) {
            ...
            // 设置 TUN 文件描述符
            try {
                LowTierJNI.setTunFd("my_instance", fd);
            } catch (RuntimeException e) {
                Log.e("LowTier", "设置 TUN FD 失败", e);
            }
        }
    ...
    private ParcelFileDescriptor establishVpnInterface() {
        ...
        builder.setSession("LowTier VPN");
...
- 使用 `adb logcat -s LowTier-JNI` 查看相关日志
- 检查 `getLastError()` 返回的错误信息

## 许可证

本项目遵循与 LowTier 主项目相同的许可证。
