...
while true; do
    ...
    if [ -f "${MODDIR}/disable" ]; then
        ...
        if pgrep -f "${LOWTIER}" >/dev/null; then
    ...
    if [ -f "${COMMAND_ARGS}" ]; then
        ...
        TZ=Asia/Shanghai "${LOWTIER}" ${FINAL_ARGS} > "${LOG_FILE}" 2>&1 &
        STR_MODE="启动参数模式"
        ...
        if grep -q "^[[:space:]]*hostname[[:space:]]*=" "${CONFIG_FILE}"; then
            TZ=Asia/Shanghai "${LOWTIER}" -c "${CONFIG_FILE}" > "${LOG_FILE}" 2>&1 &
        else
            TZ=Asia/Shanghai "${LOWTIER}" -c "${CONFIG_FILE}" --hostname "${DEVICE_HOSTNAME}" > "${LOG_FILE}" 2>&1 &
        fi
