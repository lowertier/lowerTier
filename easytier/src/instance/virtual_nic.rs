...
fn nic_packet_batch_size() -> usize {
    std::env::var("EASYTIER_DEBUG_NIC_BATCH_SIZE")
        .ok()
...
fn linux_tun_offload_enabled() -> bool {
    linux_tun_offload_configured(
        std::env::var_os("EASYTIER_ENABLE_LINUX_TUN_OFFLOAD").is_some(),
        std::env::var_os("EASYTIER_DEBUG_DISABLE_TUN_OFFLOAD").is_some(),
...
fn peer_batch_disabled() -> bool {
    *PEER_BATCH_DISABLED
        .get_or_init(|| std::env::var_os("EASYTIER_DEBUG_DISABLE_PEER_BATCH").is_some())
}
    ...
    fn do_forward_nic_to_peers_task(
        ...
        self.tasks.spawn(async move {
            if std::env::var_os("EASYTIER_DEBUG_DISABLE_PACKET_BATCH").is_some()
                || std::env::var_os("EASYTIER_DEBUG_DISABLE_NIC_BATCH").is_some()
            {
            ...
            let record_batch_stats = std::env::var_os("EASYTIER_DEBUG_BATCH_STATS").is_some();
            loop {
