...
fn udp_send_vector_disabled() -> bool {
    *UDP_SEND_VECTOR_DISABLED.get_or_init(|| {
        let explicitly_disabled = std::env::var_os("EASYTIER_DEBUG_DISABLE_UDP_VECTOR").is_some()
            || std::env::var_os("EASYTIER_DEBUG_DISABLE_UDP_SEND_VECTOR").is_some();
        #[cfg(any(target_os = "macos", target_os = "ios"))]
        {
            explicitly_disabled || std::env::var_os("EASYTIER_ENABLE_UDP_SEND_VECTOR").is_none()
        }
...
fn udp_recv_vector_disabled() -> bool {
    *UDP_RECV_VECTOR_DISABLED.get_or_init(|| {
        std::env::var_os("EASYTIER_DEBUG_DISABLE_UDP_VECTOR").is_some()
            || std::env::var_os("EASYTIER_DEBUG_DISABLE_UDP_RECV_VECTOR").is_some()
    })
}
...
fn enable_udp_gro(socket: &UdpSocket) -> io::Result<bool> {
    ...
    if std::env::var_os("EASYTIER_DEBUG_DISABLE_UDP_GRO").is_some() {
        return Ok(false);
    }
...
fn try_send_batch(
    ...
    if gso_prefix >= 2 && std::env::var_os("EASYTIER_DEBUG_DISABLE_UDP_GSO").is_none() {
...
fn try_send_gso(socket: &UdpSocket, destination: SocketAddr, buffers: &[Bytes]) -> io::Result<()> {
    ...
    // exported by every libc release supported by EasyTier.
    const SOL_UDP: libc::c_int = 17;
