...
use crate::{
    ...
    connector::udp_hole_punch::{
        common::{
            HOLE_PUNCH_PACKET_BODY_LEN, bind_ipv4_socket, send_symmetric_hole_punch_packet,
...
impl PunchSymToConeHoleClient {
    ...
    pub(crate) async fn do_hole_punching(
        ...
        let remote_socket_addr = SocketAddr::from(remote_mapped_addr);
        if !global_ctx
            .get_underlay_policy()
        ...
        if self.try_direct_connect.load(Ordering::Relaxed)
            && let Ok(tunnel) = try_connect_with_socket(
                global_ctx.clone(),
                bind_ipv4_socket(&global_ctx).await?,
                remote_socket_addr,
            )
            .await
        {
        ...
        let public_ips: Vec<Ipv4Addr> = stun_info
            .public_ip
            .iter()
            .filter_map(|x| x.parse::<Ipv4Addr>().ok())
            .filter(|ip| global_ctx.get_underlay_policy().allows_ip((*ip).into()))
            .collect();
        if public_ips.is_empty() {
        ...
        udp_array.send_with_all(&packet, remote_socket_addr).await?;
