...
impl ManualConnectorManager {
    ...
    fn strict_reconnect_ip_versions(dead_url: &url::Url) -> Vec<IpVersion> {
        match dead_url
            .host_str()
            .and_then(|host| host.parse::<IpAddr>().ok())
        {
            Some(IpAddr::V4(_)) => vec![IpVersion::V4],
            Some(IpAddr::V6(_)) => vec![IpVersion::V6],
            None => vec![IpVersion::Both],
        }
    }
...
impl ManualConnectorManager {
    ...
    async fn conn_reconnect(
        ...
        let mut ip_versions = if data.global_ctx.get_underlay_policy().is_active() {
            Self::strict_reconnect_ip_versions(&dead_url)
        } else {
            vec![]
        };
        if !ip_versions.is_empty() {
            tracing::debug!(
                ?dead_url,
                "strict reconnect skipped preflight DNS resolution"
            );
        } else if matches_scheme!(
            dead_url,
...
mod tests {
    ...
    #[test]
    fn strict_reconnect_selects_ip_version_without_resolving_hostname() {
        assert_eq!(
            ManualConnectorManager::strict_reconnect_ip_versions(
                &"tcp://peer.example:11010".parse().unwrap()
            ),
            vec![IpVersion::Both]
        );
        assert_eq!(
            ManualConnectorManager::strict_reconnect_ip_versions(
                &"tcp://192.0.2.20:11010".parse().unwrap()
            ),
            vec![IpVersion::V4]
        );
    }
    ...
    #[test]
    fn quic_reconnect_budget_covers_multiple_noisy_path_round_trips() {
        assert_eq!(
            ManualConnectorManager::reconnect_timeout(
                &"quic://peer.example:11010".parse().unwrap()
            ),
            Duration::from_secs(10)
        );
        assert_eq!(
            ManualConnectorManager::reconnect_timeout(&"udp://peer.example:11010".parse().unwrap()),
