...
LowTier 的 secure mode 已实现 Noise XX 握手 + X25519 静态公钥认证。当前节点通过 `network_secret` 双向确认身份。用户需要一种"临时凭据"机制：
...
**文件: `lowertier/src/proto/peer_rpc.proto`**
...
**文件: `lowertier/src/proto/api_instance.proto`**
...
**新文件: `lowertier/src/peers/credential_manager.rs`**
...
**文件: `lowertier/src/peers/peer_conn.rs`**
...
**文件: `lowertier/src/peers/peer_ospf_route.rs`**
...
**文件: `lowertier/src/common/global_ctx.rs`**
...
**文件: `lowertier/src/common/global_ctx.rs` - `GlobalCtxEvent`**
...
**文件: `lowertier/src/common/config.rs`**
...
**文件: `lowertier/src/peers/rpc_service.rs`**

实现 `CredentialManageRpc`，参考 `PeerManagerRpcService` 模式。

**CLI** (`lowertier-cli`):
```
lowertier-cli credential generate
  输出: credential_id=<公钥base64>  credential_secret=<私钥base64>

lowertier-cli credential revoke <credential_id>
lowertier-cli credential list
```

**临时节点启动**:
```bash
# 方式1: 直接传入凭据私钥
lowertier-core --network-name test \
  --secure-mode \
  --credential <私钥base64> \
...
| 文件 | 修改内容 |
|------|----------|
| `lowertier/src/proto/peer_rpc.proto` | `RoutePeerInfo` 加 `trusted_credential_pubkeys`; `SecureAuthLevel` 加 `CredentialAuthenticated` |
| `lowertier/src/proto/api_instance.proto` | 新增 `CredentialManageRpc` 服务及消息定义 |
| `lowertier/src/peers/credential_manager.rs` | **新文件** — 凭据管理器（密钥对生成/撤销/列表） |
| `lowertier/src/peers/mod.rs` | 导出 credential_manager |
| `lowertier/src/peers/peer_ospf_route.rs` | `new_updated_self()` 填 trusted_pubkeys; 新增 `verify_credential_peers()`; 路由表过滤 |
| `lowertier/src/peers/peer_conn.rs` | `do_noise_handshake_as_server()` 放宽 secret_proof 失败为非致命; 可选握手阶段快速拒绝 |
| `lowertier/src/peers/peer_manager.rs` | 集成 CredentialManager; 不可信 peer 断连逻辑 |
| `lowertier/src/common/global_ctx.rs` | 持有 CredentialManager; 新增 CredentialChanged 事件 |
| `lowertier/src/common/config.rs` | 新增 `--credential` 参数处理 |
| `lowertier/src/peers/rpc_service.rs` | 实现 CredentialManageRpc |
| `lowertier/src/proto/common.rs` | SecureModeConfig 可选: credential 模式识别 |
...
3. **手动测试**:
   ```bash
   # A: 管理节点
   lowertier-core -n test -s secret --secure-mode --listeners tcp://0.0.0.0:11010
   lowertier-cli credential generate  # → credential_id + credential_secret

   # C: 临时节点
   lowertier-core -n test --secure-mode --credential <私钥base64> --peers tcp://A:11010
...
**文件: `lowertier/src/peers/peer_manager.rs`**
...
lowertier-cli credential generate --groups guest,restricted --ttl 86400

# 生成允许 relay 的凭据，有效期 7 天
lowertier-cli credential generate --groups relay-node --allow-relay --ttl 604800

# 最简用法（默认 group 名为 "credential"）
lowertier-cli credential generate --ttl 3600
```
