...
impl DirectConnectorManagerData {
    ...
    async fn connect_to_public_ipv6(
        ...
        if !connector_ips.is_empty() {
            ...
            tracing::debug!(
                ?remote_url,
                "skip remote IPv6 hole-punch packet; no non-LowTier public IPv6 in STUN info"
            );
        }
    ...
    async fn spawn_direct_connect_task(
        ...
        match listener_host {
            ...
            Some(SocketAddr::V6(s_addr)) => {
                ...
                } else if self.global_ctx.is_ip_lowertier_managed_ipv6(s_addr.ip()) {
                    tracing::debug!(
                        ?listener,
                        "skip LowTier-managed IPv6 as direct-connect target"
                    );
                } else if !s_addr.ip().is_loopback() || TESTING.load(Ordering::Relaxed) {
                    if self
                        .global_ctx
...
mod tests {
    ...
    #[tokio::test]
    async fn public_ipv6_candidate_rejects_lowertier_managed_addr_even_in_tests() {
