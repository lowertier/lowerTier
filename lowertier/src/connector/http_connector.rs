...
impl HttpTunnelConnector {
    ...
    async fn handle_302_redirect(
        ...
        // 1: http(s)://example.invalid/?url=tcp://10.147.22.22:11010 (scheme is http, domain is ignored, path is splitted into proto type and addr)
    ...
    pub async fn get_redirected_connector(
        ...
        let user_agent = format!("lowertier/{}", VERSION);
...
impl super::TunnelConnector for HttpTunnelConnector {
    async fn connect(&mut self) -> Result<Box<dyn Tunnel>, TunnelError> {
        if self.global_ctx.get_underlay_policy().is_active() {
            return Err(TunnelError::UnderlayPolicyDenied(
                "HTTP and HTTPS peer discovery is disabled by strict underlay policy".into(),
            ));
        }
        let mut conn = self
...
mod tests {
    ...
    #[tokio::test]
    async fn strict_policy_rejects_http_discovery_before_request() {
        let global_ctx = get_mock_global_ctx_with_network(None);
        let mut flags = global_ctx.get_flags();
        flags.underlay_deny_interfaces = vec!["tailscale0".into()];
        global_ctx.set_flags(flags);
        let mut connector =
            HttpTunnelConnector::new("http://127.0.0.1:9".parse().unwrap(), global_ctx);
