...
use crate::{
    ...
    tunnel::{
        TunnelConnector as _, TunnelError, TunnelListener as _,
        common::bind,
...
async fn select_local_addr(peer_mgr: &Arc<PeerManager>, is_v6: bool) -> Result<SocketAddr, Error> {
    let global_ctx = peer_mgr.get_global_ctx();
    let policy = global_ctx.get_underlay_policy();
    let candidates = if policy.is_active() {
            ...
            .call()
        {
            Ok(listener) => {
                ...
                tracing::debug!(
                    ?bind_addr,
                    ?local_addr,
                    "tcp hole punch selected local address"
                );
                return Ok(local_addr);
            }
            Err(error) => tracing::debug!(?bind_addr, ?error, "tcp hole punch bind failed"),
        }
    }

    Err(TunnelError::UnderlayPolicyDenied(format!(
        "no allowed local {} source for TCP hole punching",
...
async fn try_connect_to_remote(
    ...
    connector.set_bind_addrs(vec![local_addr]);
    connector.set_underlay_policy(peer_mgr.get_global_ctx().get_underlay_policy());
...
impl TcpHolePunchRpc for TcpHolePunchServer {
    ...
    async fn exchange_mapped_addr(
        ...
        {
            return Err(
                anyhow::anyhow!("connector_mapped_addr is denied by underlay policy").into(),
            );
        }

        let is_v6 = a_mapped_addr.is_ipv6();
        let local_addr = select_local_addr(&self.peer_mgr, is_v6).await?;
        ...
        if !self
            ...
            .get_global_ctx()
            .get_underlay_policy()
            .allows_ip(mapped_addr.ip())
        {
            return Err(anyhow::anyhow!("mapped TCP address is denied by underlay policy").into());
        }
        ...
        self.tasks.lock().unwrap().spawn(async move {
            let _ = try_connect_to_remote(peer_mgr, a_mapped_addr, local_addr, true, 5).await;
        });
...
impl TcpHolePunchConnectorData {
    ...
    async fn do_punch_as_initiator(&self, dst_peer_id: PeerId) -> Result<(), Error> {
        ...
        let local_addr = select_local_addr(&self.peer_mgr, false).await?;
        let local_port = local_addr.port();
        let mapped_addr = global_ctx
            ...
            .await
            .with_context(|| "failed to get tcp port mapping")?;
        if !global_ctx.get_underlay_policy().allows_ip(mapped_addr.ip()) {
            return Err(anyhow::anyhow!(
                "mapped TCP address is denied by underlay policy"
            ));
        }
        ...
        if !global_ctx
            .get_underlay_policy()
            .allows_remote(remote_mapped_addr.ip())
        {
            return Err(anyhow::anyhow!(
                "remote mapped TCP address is denied by underlay policy"
            ));
        }
        tracing::info!(
            dst_peer_id,
