...
async fn set_bind_addr_for_peer_connector(
    ...
    if is_ipv4 {
        ...
        for ipv6 in ips.interface_ipv6s.iter().chain(ips.public_ipv6.iter()) {
            ...
            if global_ctx.is_ip_lowertier_managed_ipv6(&ipv6) {
                continue;
            }
...
async fn lowertier_managed_ipv6_source_for_dst(
    global_ctx: &ArcGlobalCtx,
    ...
    Ok(global_ctx
        .is_ip_lowertier_managed_ipv6(&local_ip)
        .then_some(local_ip))
}
...
async fn ipv6_connector_reject_reason(
    ...
    if global_ctx.is_ip_lowertier_managed_ipv6(v6_addr.ip()) {
        return Ok(Some(format!(
            "{} resolves to LowTier-managed IPv6 {}",
            url,
            v6_addr.ip()
        )));
    }

    match lowertier_managed_ipv6_source_for_dst(global_ctx, v6_addr).await {
        Ok(Some(local_ip)) => Ok(Some(format!(
            "{} would use LowTier-managed IPv6 {} as local source for {}",
...
async fn resolve_connector_socket_addr(
    ...
    let mut rejected_underlay_reason = None;
    ...
    {
        if let Some(rule) = underlay_policy.denied_ip_rule(addr.ip()) {
            rejected_underlay_reason = Some(format!(
                "{} resolves to {} which matches denied underlay CIDR {}",
                url,
                addr.ip(),
                rule
            ));
    ...
    if usable_addrs.is_empty() {
        if let Some(reason) = rejected_underlay_reason {
...
pub async fn create_connector_by_url(
    ...
    let mut connector: Box<dyn TunnelConnector + 'static> = match scheme {
        TunnelScheme::Ip(scheme) => {
            ...
            connector.set_underlay_policy(underlay_policy.clone());
            if global_ctx.config.get_flags().bind_device || underlay_policy.is_active() {
...
mod tests {
    ...
    #[tokio::test]
    async fn connector_rejects_lowertier_managed_ipv6_destination() {
    ...
    #[tokio::test]
    async fn connector_rejects_denied_destination_before_connect() {
        let global_ctx = get_mock_global_ctx();
        ...
        let ret =
            create_connector_by_url("tcp://100.108.186.13:11010", &global_ctx, IpVersion::V4).await;

        let Err(crate::common::error::Error::InvalidUrl(message)) = ret else {
            panic!("expected denied destination error");
        };
        assert!(message.contains("100.108.186.13"));
        assert!(message.contains("underlay"));
    }

    #[tokio::test]
    async fn strict_policy_rejects_hostname_before_system_resolution() {
        let global_ctx = get_mock_global_ctx();
        let mut flags = global_ctx.get_flags();
        ...
        let ret =
            create_connector_by_url("tcp://peer.example:11010", &global_ctx, IpVersion::V4).await;

        assert!(matches!(
            ret,
            Err(crate::common::error::Error::TunnelError(
