...
impl PunchBothEasySymHoleServer {
    ...
    pub(crate) async fn send_punch_packet_both_easy_sym(
        ...
        if !global_ctx
            .get_underlay_policy()
            .allows_ip(cur_mapped_addr.ip())
        {
            return Err(
                anyhow::anyhow!("local UDP mapped address is denied by underlay policy").into(),
            );
        }
        ...
        let public_ip = std::net::Ipv4Addr::from(public_ips);
        if !global_ctx
            .get_underlay_policy()
            .allows_remote(public_ip.into())
        {
            return Err(
                anyhow::anyhow!("remote UDP public address is denied by underlay policy").into(),
            );
        }
        ...
        let task = tokio::spawn(async move {
            ...
            while start_time.elapsed() < Duration::from_millis(wait_time_ms as u64) {
                if let Err(e) = udp_array
                    .send_with_all(
                        &punch_packet,
                        SocketAddr::V4(SocketAddrV4::new(public_ip, request.dst_port_num as u16)),
                    )
                    .await
                {
    ...
    pub(crate) async fn do_hole_punching(
        ...
        *is_busy = false;
        ...
        if !global_ctx
            ...
            .allows_ip(cur_mapped_addr.ip())
        {
            anyhow::bail!("local UDP mapped address is denied by underlay policy");
        }
