...
impl DnsTunnelConnector {
    ...
    pub async fn handle_srv_record(
        ...
        let srv_domains = IpScheme::VARIANTS
            .iter()
            .map(|s| (s, format!("_lowertier._{}.{}", s, domain_name)))
            .collect::<Vec<_>>();
    ...
    async fn connect(&mut self) -> Result<Box<dyn Tunnel>, TunnelError> {
        if self.global_ctx.get_underlay_policy().is_active() {
            return Err(TunnelError::UnderlayPolicyDenied(
                "TXT and SRV peer discovery is disabled by strict underlay policy".into(),
            ));
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn strict_policy_rejects_txt_discovery_before_query() {
        let global_ctx = get_mock_global_ctx();
        let mut flags = global_ctx.get_flags();
        flags.underlay_deny_interfaces = vec!["tailscale0".into()];
        global_ctx.set_flags(flags);
        let mut connector =
            DnsTunnelConnector::new("txt://peer.example".parse().unwrap(), global_ctx);
    ...
    #[tokio::test]
    async fn test_txt() {
        let url = "txt://stun.example.invalid";
        let global_ctx = get_mock_global_ctx();
    ...
    #[tokio::test]
    async fn test_srv() {
        let url = "srv://peer.example.invalid";
        let global_ctx = get_mock_global_ctx();
        let mut connector = DnsTunnelConnector::new(url.parse().unwrap(), global_ctx);
        connector.set_ip_version(IpVersion::V4);
        for _ in 0..5 {
