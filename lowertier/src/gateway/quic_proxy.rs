...
impl QuicProxy {
    ...
    pub async fn run(&mut self, src: bool, dst: bool) {
        ...
        let server = match server_config(&flags) {
            Ok(config) => config,
            Err(error) => {
                error!(?error, "invalid QUIC server transport configuration");
                return;
            }
        };
        let client = match client_config(&flags) {
            Ok(config) => config,
            Err(error) => {
                error!(?error, "invalid QUIC client transport configuration");
                return;
            }
        };
...
mod tests {
    ...
    fn endpoint() -> (Endpoint, Endpoint) {
        ...
        let server_config = server_config(&flags).unwrap();
        let client_config = client_config(&flags).unwrap();
