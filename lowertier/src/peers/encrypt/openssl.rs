...
impl OpenSslCipher {
    ...
    pub fn new_chacha20_poly1305(key: [u8; 32]) -> Self {
        Self {
            cipher: OpenSslEnum::ChaCha20Poly1305(key),
        }
    }
    ...
    fn get_cipher_and_key(&self) -> (Cipher, &[u8]) {
        match &self.cipher {
            ...
            OpenSslEnum::ChaCha20Poly1305(key) => (Cipher::chacha20_poly1305(), key.as_slice()),
        }
    }
}
...
mod tests {
    ...
    fn test_openssl_chacha20() {
        let key = [3u8; 32];
        let cipher = OpenSslCipher::new_chacha20_poly1305(key);
        run_cipher_test_with_nonce(cipher);
    }
}
