    ...
    pub async fn send_msg_batch(
        ...
        if self.is_secure_mode_enabled() {
            let session = match self.ensure_session(dst_peer_id, policy.clone()).await {
                ...
                Err(_) => {
                    for packet in batch {
                        self.buffer_pending_packet(dst_peer_id, packet, policy.clone());
                    }
                    return Ok(());
                }
            };
            ...
            if parallel_crypto_enabled(batch.len()) {
                ...
                for packet in batch.iter_mut() {
                    session
                        .encrypt_payload(my_peer_id, dst_peer_id, packet)
                        .map_err(|error| Error::RouteError(Some(format!("{error:?}"))))?;
                }
            }
        }

        self.send_via_next_hop_batch(batch, dst_peer_id, policy)
            .await
    }
