...
use crate::tunnel::{
    ...
    common::{TunnelWrapper, eligible_bind_addrs, ensure_local_allowed, ensure_remote_allowed},
...
fn select_faketcp_source(
    policy: &UnderlayPolicy,
    bind_addrs: &[SocketAddr],
    remote_addr: SocketAddr,
) -> Result<Option<IpAddr>, TunnelError> {
    let eligible = eligible_bind_addrs(policy, bind_addrs, remote_addr)?;
    Ok(eligible.first().map(SocketAddr::ip))
}
...
impl TunnelConnector for FakeTcpTunnelConnector {
    async fn connect(&mut self) -> Result<Box<dyn Tunnel>, TunnelError> {
        ...
        ensure_remote_allowed(&self.underlay_policy, remote_addr)?;
        let local_ip = select_faketcp_source(&self.underlay_policy, &self.bind_addrs, remote_addr)?
            .or_else(|| get_local_ip_for_destination(remote_addr.ip()))
            .ok_or(TunnelError::InternalError("Failed to get local ip".into()))?;
        ensure_local_allowed(&self.underlay_policy, SocketAddr::new(local_ip, 0))?;
        ...
        os_socket.bind(SocketAddr::new(local_ip, 0))?;
        ...
        if !self
            .underlay_policy
            .allows_local_endpoint(Some(&interface_name), local_ip)
        {
            return Err(TunnelError::UnderlayPolicyDenied(format!(
                "FakeTCP local address {local_ip} on interface {interface_name} is denied"
            )));
        }
...
mod tests {
    use crate::{
        ...
        tunnel::{TunnelConnector, common::tests::_tunnel_pingpong},
    };

    use super::*;

    #[test]
    fn active_policy_selects_only_an_eligible_faketcp_source() {
        let policy = UnderlayPolicy::new(&[], &["100.64.0.0/10".into()]).unwrap();
        let bind_addrs = [
            "100.100.10.20:0".parse().unwrap(),
            "192.0.2.20:0".parse().unwrap(),
        ];

        let source =
            select_faketcp_source(&policy, &bind_addrs, "198.51.100.8:443".parse().unwrap())
                .unwrap();

        assert_eq!(source, Some("192.0.2.20".parse().unwrap()));
    }

    #[tokio::test]
    async fn denied_faketcp_remote_fails_before_opening_raw_transport() {
        let mut connector =
            FakeTcpTunnelConnector::new("faketcp://100.100.10.20:443".parse().unwrap());
        connector.set_resolved_addr("100.100.10.20:443".parse().unwrap());
        connector.set_bind_addrs(vec!["192.0.2.20:0".parse().unwrap()]);
