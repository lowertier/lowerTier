...
#[derive(Debug, Clone)]
enum WgType {
    // used by lowertier peer, need remove/add ip header for in/out wg msg
    InternalUse,
...
impl WgTunnelConnector {
    ...
    async fn connect_with_socket(
        ...
        ensure_remote_allowed(&underlay_policy, addr)?;
        ensure_local_allowed(&underlay_policy, udp.local_addr()?)?;
        let local_addr = udp
            .local_addr()
        ...
        ensure_remote_allowed(&underlay_policy, recv_addr)?;
...
impl super::TunnelConnector for WgTunnelConnector {
    ...
    async fn connect(&mut self) -> Result<Box<dyn Tunnel>, TunnelError> {
        ...
        let mut bind_addrs = eligible_bind_addrs(&self.underlay_policy, &self.bind_addrs, addr)?;
        if bind_addrs.is_empty() {
            bind_addrs.push(if addr.is_ipv4() {
...
pub mod tests {
    ...
    #[tokio::test]
    async fn wireguard_active_policy_without_bind_sources_fails_closed() {
        let (_, client_cfg) = create_wg_config();
        let mut connector = WgTunnelConnector::new("wg://127.0.0.1:9".parse().unwrap(), client_cfg);
