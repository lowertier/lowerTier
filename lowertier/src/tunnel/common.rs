...
use crate::common::{netns::NetNS, underlay_policy::UnderlayPolicy};
...
pub fn ensure_remote_allowed(
    policy: &UnderlayPolicy,
    remote: SocketAddr,
) -> Result<(), TunnelError> {
    if let Some(rule) = policy.denied_ip_rule(remote.ip()) {
        return Err(TunnelError::UnderlayPolicyDenied(format!(
            "remote address {} matches denied CIDR {}",
            remote.ip(),
            rule
        )));
    }
...
pub fn ensure_local_allowed(policy: &UnderlayPolicy, local: SocketAddr) -> Result<(), TunnelError> {
    if let Some(rule) = policy.denied_ip_rule(local.ip()) {
        return Err(TunnelError::UnderlayPolicyDenied(format!(
            "local address {} matches denied CIDR {}",
            local.ip(),
            rule
        )));
    }
    if policy.has_interface_rules() {
        let interface = get_interface_name_by_ip(&local.ip()).ok_or_else(|| {
            TunnelError::UnderlayPolicyDenied(format!(
                "cannot prove the interface for local address {}",
                local.ip()
            ))
        })?;
        if let Some(rule) = policy.denied_interface_rule(&interface) {
            return Err(TunnelError::UnderlayPolicyDenied(format!(
                "local address {} belongs to denied interface {}",
                local.ip(),
                rule
            )));
        }
...
pub fn eligible_bind_addrs(
    ...
    let eligible = bind_addrs
        ...
        .filter(|addr| {
            ...
            let interface = if policy.has_interface_rules() {
                get_interface_name_by_ip(&addr.ip())
            } else {
                None
            };
            policy.allows_local_endpoint(interface.as_deref(), addr.ip())
        })
        .collect::<Vec<_>>();

    if policy.is_active() && eligible.is_empty() {
        return Err(TunnelError::UnderlayPolicyDenied(format!(
            "no allowed local {} source address is available for {}",
...
impl<W, C: ZCPacketToBytes + Send + 'static> FramedWriter<W, C> {
    ...
    pub fn new_with_converter_and_max_buffer_count(
        writer: W,
        converter: C,
        max_buffer_count: usize,
    ) -> Self {
...
pub mod tests {
    ...
    #[test]
    fn interface_policy_fails_closed_when_local_interface_cannot_be_proven() {
        let policy = UnderlayPolicy::new(&["tailscale0".into()], &[]).unwrap();

        let result = super::ensure_local_allowed(&policy, "192.0.2.250:1".parse().unwrap());

        assert!(matches!(result, Err(TunnelError::UnderlayPolicyDenied(_))));
    }

    #[test]
    fn bind_candidate_with_unproven_interface_is_not_eligible() {
        let policy = UnderlayPolicy::new(&["tailscale0".into()], &[]).unwrap();

        let result = super::eligible_bind_addrs(
            &policy,
            &["192.0.2.250:0".parse().unwrap()],
            "198.51.100.8:443".parse().unwrap(),
        );
