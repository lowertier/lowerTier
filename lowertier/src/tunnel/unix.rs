...
mod tests {
    ...
    async fn unix_socket_pingpong() {
        let listener =
            UnixSocketTunnelListener::new("unix:///tmp/lowertier-test.sock".parse().unwrap());
        let connector =
    ...
    async fn unlink_on_drop() {
        let listener = UnixSocketTunnelListener::new(
            "unix:///tmp/lowertier-test-exists.sock".parse().unwrap(),
        );
        ...
        let mut listener = UnixSocketTunnelListener::new(
            "unix:///tmp/lowertier-test-exists.sock".parse().unwrap(),
        );
    ...
    async fn bind_file_exists() {
        ...
        let path = "/tmp/lowertier-test-exists.sock";
