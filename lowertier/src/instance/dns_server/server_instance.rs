// single-instance server in one machine, every lowertier instance that has ip address and tun device will try to create a server instance.
...
// all the clients will exit and let the lowertier instance to launch a new server instance.
