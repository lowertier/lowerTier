...
use lowertier::service_manager::{Service, ServiceInstallOptions};
...
#[derive(Parser, Debug)]
#[command(name = "lowertier-cli", author, version = LOWTIER_VERSION, about, long_about = None)]
struct Cli {
    #[arg(
        short = 'p',
        long,
        default_value = "127.0.0.1:15888",
        help = "lowertier-core rpc portal address"
    )]
...
enum SubCommand {
    ...
    VpnPortal,
    #[command(about = "inspect self lowertier-core status")]
    Node(NodeArgs),
    #[command(about = "manage lowertier-core as a system service")]
    Service(ServiceArgs),
...
enum ServiceSubCommand {
    #[command(about = "register lowertier-core as a system service")]
    Install(InstallArgs),
    #[command(about = "unregister lowertier-core system service")]
    Uninstall,
    #[command(about = "check lowertier-core system service status")]
    Status,
    #[command(about = "start lowertier-core system service")]
    Start,
    #[command(about = "stop lowertier-core system service")]
    Stop,
}
...
struct InstallArgs {
    ...
    #[arg(long, help = "path to lowertier-core binary")]
    core_path: Option<PathBuf>,

    #[arg(long, help = "working directory for the lowertier-core service")]
    service_work_dir: Option<PathBuf>,

    #[arg(
        trailing_var_arg = true,
        allow_hyphen_values = true,
        help = "args to pass to lowertier-core"
    )]
...
impl<'a> CommandHandler<'a> {
        ...
        impl From<PeerRoutePair> for PeerTableItem {
            fn from(p: PeerRoutePair) -> Self {
                ...
                PeerTableItem {
                    ...
                    ipv4: route
                        .ipv4_addr
                        .map(|ip: lowertier::proto::common::Ipv4Inet| {
    ...
    async fn handle_peer_ipv6(&self) -> Result<(), Error> {
        ...
        fn fmt_ipv6_inet(value: Option<lowertier::proto::common::Ipv6Inet>) -> String {
            value
    ...
    match cli.sub_command {
            ...
            match service_args.sub_command {
                ServiceSubCommand::Install(install_args) => {
                    ...
                    let bin_path = std::fs::canonicalize(bin_path).map_err(|e| {
                        anyhow::anyhow!("failed to get lowertier core application: {}", e)
                    })?;
        ...
        SubCommand::GenAutocomplete { shell } => {
            ...
            if let Some(shell) = shell.to_shell() {
                lowertier::print_completions(shell, &mut cmd, "lowertier-cli");
            } else {
                // Handle Nushell
                lowertier::print_nushell_completions(&mut cmd, "lowertier-cli");
            }
        }
    }
