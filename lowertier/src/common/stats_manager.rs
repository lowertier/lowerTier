...
pub struct CounterHandle {
    ...
    _key: Arc<MetricKey>, // Keep shared key for allocation-free handle clones.
}
...
mod tests {
    ...
    #[tokio::test]
    async fn cloned_counter_handles_share_the_metric_key_allocation() {
        let stats = StatsManager::new();
        let counter = stats.get_counter(
            MetricName::TrafficBytesTx,
            LabelSet::new().with_label("network", "allocation-test"),
        );
        let clone = counter.clone();

        assert!(Arc::ptr_eq(&counter._key, &clone._key));
    }
