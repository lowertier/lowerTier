...
const UPNP_DESCRIPTION: &str = "LowTier udp hole punch";
...
pub async fn resolve_udp_public_addr(
    ...
    let (mapped_addrs, lease) =
        resolve_udp_public_addrs(global_ctx, local_listener, socket).await?;
    let mapped_addr = mapped_addrs
        .into_iter()
        .next()
        .ok_or_else(|| anyhow::anyhow!("no UDP public address was resolved"))?;
    Ok((mapped_addr, lease))
}

pub async fn resolve_udp_public_addrs(
    global_ctx: ArcGlobalCtx,
    local_listener: &url::Url,
    socket: Arc<UdpSocket>,
) -> anyhow::Result<(Vec<SocketAddr>, Option<UdpPortMappingLease>)> {
    ...
    if let Some(port_mapping) = port_mapping.as_ref() {
        let mapped_addr = mapped_addrs[0];
        ...
        tracing::debug!(
            %local_listener,
            ?mapped_addrs,
            "udp public addresses resolved without port mapping"
        );
    }

    Ok((mapped_addrs, port_mapping))
}
...
async fn try_start_udp_port_mapping(
    ...
    if global_ctx.get_underlay_policy().is_active() {
        tracing::debug!(
            %local_listener,
            "skip UPnP and NAT-PMP because strict underlay policy is active"
        );
        return Ok(None);
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn strict_underlay_policy_skips_unbound_port_mapping_clients() {
        let global_ctx = get_mock_global_ctx();
        let mut flags = global_ctx.get_flags();
        flags.underlay_deny_interfaces = vec!["tailscale0".into()];
        global_ctx.set_flags(flags);
        super::reset_udp_port_mapping_attempts_for_test();

        let result = super::try_start_udp_port_mapping(
            &global_ctx,
            &"udp://192.168.1.10:11010".parse().unwrap(),
        )
        .await
        ...
        assert_eq!(super::udp_port_mapping_attempts_for_test(), 0);
    }
}
