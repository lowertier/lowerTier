...
pub fn gen_default_flags() -> Flags {
    ...
    Flags {
        ...
        socket_mark: None,
        underlay_deny_interfaces: Vec::new(),
        underlay_deny_cidrs: Vec::new(),
...
pub fn validate_flags(flags: &Flags) -> anyhow::Result<()> {
    ...
    if !(1..=86_400).contains(&flags.l2_fdb_age_seconds) {
        anyhow::bail!("l2_fdb_age_seconds must be between 1 and 86400");
    }

    match flags.quic_congestion.as_str() {
        "adaptive" | "bbr" => {}
        "brutal" if flags.quic_brutal_send_bps > 0 => {}
        "brutal" => {
            anyhow::bail!(
                "quic_brutal_send_bps must be greater than zero when quic_congestion is brutal"
            )
        }
        controller => {
            anyhow::bail!(
                "unsupported quic_congestion {controller:?}; expected \"adaptive\", \"bbr\", or \"brutal\""
            )
        }
    }

    const MAX_QUIC_VARINT: u64 = (1_u64 << 62) - 1;
    if flags.quic_initial_receive_window == 0 || flags.quic_initial_receive_window > MAX_QUIC_VARINT
    {
        anyhow::bail!("quic_initial_receive_window must be between 1 and {MAX_QUIC_VARINT}");
    }
    if flags.quic_receive_window < flags.quic_initial_receive_window
        || flags.quic_receive_window > MAX_QUIC_VARINT
    {
        anyhow::bail!(
            "quic_receive_window must be at least quic_initial_receive_window and no more than {MAX_QUIC_VARINT}"
        );
    }
...
pub mod tests {
    ...
    #[test]
    fn encryption_default_is_chacha20_poly1305() {
        assert_eq!(
            EncryptionAlgorithm::default().to_string(),
            "chacha20-poly1305"
        );
    }
    ...
    #[test]
    fn encryption_rejects_xor_as_an_algorithm() {
        assert!("xor".parse::<EncryptionAlgorithm>().is_err());
    }
...
"#,
    ...
    fn underlay_and_quic_flags_have_safe_defaults() {
        ...
        assert!(flags.underlay_deny_interfaces.is_empty());
        assert!(flags.underlay_deny_cidrs.is_empty());
    ...
    #[test]
    fn underlay_and_quic_flags_roundtrip_through_toml() {
        let cfg = TomlConfigLoader::new_from_str(
            r#"
[flags]
...
"#,
        ...
        assert_eq!(flags.underlay_deny_interfaces, ["utun5"]);
        assert_eq!(
            flags.underlay_deny_cidrs,
            ["100.64.0.0/10", "fd7a:115c:a1e0::/48"]
        );
        assert_eq!(flags.quic_congestion, "brutal");
        assert_eq!(flags.quic_brutal_send_bps, 50_000_000);
        assert!(!flags.quic_brutal_loss_compensation);
        ...
        assert_eq!(reloaded.get_flags(), flags);
    }

    #[test]
    fn rejects_unknown_quic_congestion_controller() {
        let err = TomlConfigLoader::new_from_str(
            r#"
[flags]
quic_congestion = "turbo"
"#,
        )
        .unwrap_err();

        assert!(err.to_string().contains("quic_congestion"));
        assert!(err.to_string().contains("turbo"));
    }

    #[test]
    fn accepts_adaptive_quic_congestion_controller() {
        let mut flags = gen_default_flags();
    ...
    fn rejects_unsupported_alternate_fec_profile() {
        ...
        let error = validate_flags(&flags).unwrap_err();
        assert!(error.to_string().contains("quic_datagram_fec_parity"));
    }
    ...
    fn rejects_invalid_l2_flags() {
        for (key, value) in [
            ...
            ("l2_fdb_age_seconds", "86401"),
        ] {
            let err =
                TomlConfigLoader::new_from_str(&format!("[flags]\n{key} = {value}\n")).unwrap_err();
            assert!(
                err.to_string().contains(key),
                "expected error for {key}={value}, got {err}"
            );
        }
    }

    #[test]
    fn brutal_requires_positive_send_rate() {
        let err = TomlConfigLoader::new_from_str(
            r#"
[flags]
quic_congestion = "brutal"
...
"#,
        ...
        assert!(err.to_string().contains("quic_brutal_send_bps"));
    }

    #[test]
    fn dump_preserves_flags_that_differ_from_lowertier_defaults() {
    ...
    fn test_stun_servers_config() {
        ...
        let custom_servers = vec!["txt:stun.example.invalid".to_string()];
...
stun_servers = [
    ...
    "txt:stun.example.invalid"
        ...
        assert_eq!(stun_servers[2], "txt:stun.example.invalid");
    }
...
"#;
    ...
    /// `lowertier/src/rpc_service/instance_manage.rs` 中实现
    #[tokio::test]
