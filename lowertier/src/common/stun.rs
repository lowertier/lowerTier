...
const DEFAULT_UDP_V6_STUN_SERVERS: &[&str] = &["stun.cloudflare.com:3478"];
...
mod tests {
    ...
    #[test]
    fn default_stun_servers_use_direct_hosts() {
        assert!(
            DEFAULT_UDP_STUN_SERVERS
                .iter()
                .all(|server| !server.starts_with("txt:"))
        );
        assert!(
            DEFAULT_UDP_V6_STUN_SERVERS
                .iter()
    ...
    #[tokio::test]
    #[ignore = "requires a configured TXT STUN record"]
    async fn test_txt_public_stun_server() {
        let stun_servers = vec!["txt:stun.example.invalid".to_string()];
        let detector = UdpNatTypeDetector::new(stun_servers, 1);
