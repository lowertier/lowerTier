...
use super::{netns::NetNS, stun::StunInfoCollectorTrait, underlay_policy::UnderlayPolicy};

pub const CACHED_IP_LIST_TIMEOUT_SEC: u64 = 60;

fn allowed_interface_ips(
    ...
    for interface in interfaces {
        if !policy.allows_interface(&interface.name) {
            tracing::debug!(interface = %interface.name, "underlay policy removed interface");
            continue;
        }

        for network in &interface.ips {
            let addr = network.ip();
            if !policy.allows_ip(addr) {
                tracing::debug!(interface = %interface.name, %addr, "underlay policy removed address");
                continue;
            }
...
fn fallback_source_allowed(
    ...
    let interface = interfaces
        .iter()
        .find(|interface| interface.ips.iter().any(|network| network.ip() == addr))
        .map(|interface| interface.name.as_str());
    policy.allows_local_endpoint(interface, addr)
}
...
impl IPCollector {
    pub fn new<T: StunInfoCollectorTrait + 'static>(
        net_ns: NetNS,
        stun_info_collector: T,
        underlay_policy: Arc<ArcSwap<UnderlayPolicy>>,
    ) -> Self {
        Self {
    ...
    pub async fn collect_ip_addrs(&self) -> GetIpListResponse {
        ...
        if task.is_empty() {
            ...
            *cached_ip_list.write().await = Self::do_collect_local_ip_addrs(
                self.net_ns.clone(),
            ...
            task.spawn(async move {
                ...
                loop {
                    if last_fetch_iface_time.elapsed().as_secs() > CACHED_IP_LIST_TIMEOUT_SEC {
                        let ifaces = Self::do_collect_local_ip_addrs(
                            net_ns.clone(),
                            underlay_policy.load_full(),
                        )
                        .await;
                    ...
                    for ip in stun_info.public_ip.iter() {
                        ...
                        if !underlay_policy.load().allows_ip(ip_addr) {
                            tracing::debug!(%ip_addr, "underlay policy removed STUN address");
                            continue;
                        }
    ...
    #[tracing::instrument(skip(net_ns, policy))]
    async fn do_collect_local_ip_addrs(
        net_ns: NetNS,
        policy: Arc<UnderlayPolicy>,
    ) -> GetIpListResponse {
        let mut ret = GetIpListResponse::default();

        let ipv4_interfaces = Self::collect_interfaces(net_ns.clone(), true).await;
        let all_interfaces = Self::collect_interfaces(net_ns.clone(), false).await;
        let (ipv4, _) = allowed_interface_ips(&ipv4_interfaces, &policy);
        let (_, ipv6) = allowed_interface_ips(&all_interfaces, &policy);
        ret.interface_ipv4s = ipv4.into_iter().map(Into::into).collect();
        ret.interface_ipv6s = ipv6.into_iter().map(Into::into).collect();

        let _g = net_ns.guard();
        if let Ok(v4_addr) = local_ipv4().await {
            tracing::trace!("got local ipv4: {}", v4_addr);
            if fallback_source_allowed(v4_addr.into(), &all_interfaces, &policy)
                && !ret.interface_ipv4s.contains(&v4_addr.into())
            {
        ...
        if let Ok(v6_addr) = local_ipv6().await {
            ...
            if fallback_source_allowed(v6_addr.into(), &all_interfaces, &policy)
                && !ret.interface_ipv6s.contains(&v6_addr.into())
            {
...
mod tests {
    use super::*;
    use crate::common::underlay_policy::UnderlayPolicy;
    ...
    fn iface(name: &str, ips: &[&str]) -> NetworkInterface {
        NetworkInterface {
            ...
            ips: ips
                .iter()
                .map(|ip| ip.parse::<IpNetwork>().unwrap())
                .collect(),
            flags: 0,
        }
    }
    ...
    #[test]
    fn allowed_interface_ips_remove_denied_tailscale_paths() {
        let interfaces = vec![
            iface("en0", &["172.25.3.110/16", "2001:db8:1::10/64"]),
            iface(
                "utun5",
                &["100.108.186.13/32", "fd7a:115c:a1e0::1b32:ba0d/128"],
            ),
            iface("utun13", &["10.126.126.2/24"]),
        ];
        ...
        let (ipv4, ipv6) = allowed_interface_ips(&interfaces, &policy);

        assert_eq!(
            ipv4,
            [
                "172.25.3.110".parse::<Ipv4Addr>().unwrap(),
                "10.126.126.2".parse::<Ipv4Addr>().unwrap()
            ]
        );
        assert_eq!(ipv6, ["2001:db8:1::10".parse::<Ipv6Addr>().unwrap()]);
    }

    #[test]
    fn fallback_source_requires_known_allowed_interface() {
        let interfaces = vec![
            iface("en0", &["172.25.3.110/16"]),
            iface("utun5", &["100.108.186.13/32"]),
        ];
        let policy = UnderlayPolicy::new(&["utun5".into()], &[]).unwrap();

        assert!(fallback_source_allowed(
            "172.25.3.110".parse().unwrap(),
            &interfaces,
            &policy
        ));
        assert!(!fallback_source_allowed(
            "100.108.186.13".parse().unwrap(),
            &interfaces,
            &policy
        ));
        assert!(!fallback_source_allowed(
