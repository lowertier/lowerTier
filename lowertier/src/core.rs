...
#[derive(Parser, Debug)]
#[command(name = "lowertier-core", author, version = LOWTIER_VERSION , about, long_about = None)]
struct Cli {
    #[arg(
        short = 'w',
        long,
...
struct NetworkOptions {
    ...
    #[arg(
        long,
        env = "ET_UNDERLAY_DENY_INTERFACES",
        value_delimiter = ',',
        help = t!("core_clap.underlay_deny_interfaces").to_string(),
        num_args = 0..
    )]
    underlay_deny_interfaces: Vec<String>,

    #[arg(
        long,
        env = "ET_UNDERLAY_DENY_CIDRS",
        value_delimiter = ',',
        help = t!("core_clap.underlay_deny_cidrs").to_string(),
        num_args = 0..
    )]
    ...
    #[arg(
        long,
        env = "ET_L2_FDB_CAPACITY",
        help = t!("core_clap.l2_fdb_capacity").to_string()
    )]
    l2_fdb_capacity: Option<u32>,

    #[arg(
        long,
        env = "ET_L2_FDB_AGE_SECONDS",
        help = t!("core_clap.l2_fdb_age_seconds").to_string()
    )]
    l2_fdb_age_seconds: Option<u64>,

    #[arg(
        long,
        env = "ET_L2_FLOOD_BPS",
        help = t!("core_clap.l2_flood_bps").to_string()
    )]
    l2_flood_bps: Option<u64>,
    ...
    quic_congestion: Option<String>,

    #[arg(
        long,
        env = "ET_QUIC_BRUTAL_SEND_BPS",
        help = t!("core_clap.quic_brutal_send_bps").to_string()
    )]
    quic_brutal_send_bps: Option<u64>,

    #[arg(
        long,
        env = "ET_QUIC_BRUTAL_LOSS_COMPENSATION",
        help = t!("core_clap.quic_brutal_loss_compensation").to_string(),
        num_args = 0..=1,
        default_missing_value = "true"
    )]
    quic_brutal_loss_compensation: Option<bool>,

    #[arg(
        long,
        env = "ET_QUIC_INITIAL_RECEIVE_WINDOW",
        help = t!("core_clap.quic_initial_receive_window").to_string()
    )]
    quic_initial_receive_window: Option<u64>,

    #[arg(
        long,
        env = "ET_QUIC_RECEIVE_WINDOW",
        help = t!("core_clap.quic_receive_window").to_string()
    )]
    quic_receive_window: Option<u64>,

    #[arg(
        long,
        env = "ET_QUIC_DATAGRAM_FEC_PARITY",
        help = t!("core_clap.quic_datagram_fec_parity").to_string()
    )]
    quic_datagram_fec_parity: Option<u32>,
    ...
    quic_datagram_alternate_path_parity: Option<bool>,
...
impl NetworkOptions {
    ...
    fn merge_into(&self, cfg: &TomlConfigLoader) -> anyhow::Result<()> {
        ...
        if !self.underlay_deny_interfaces.is_empty() {
            f.underlay_deny_interfaces = self.underlay_deny_interfaces.clone();
        }
        ...
        f.quic_brutal_send_bps = self.quic_brutal_send_bps.unwrap_or(f.quic_brutal_send_bps);
        f.quic_brutal_loss_compensation = self
            .quic_brutal_loss_compensation
            .unwrap_or(f.quic_brutal_loss_compensation);
        f.quic_initial_receive_window = self
            .quic_initial_receive_window
            .unwrap_or(f.quic_initial_receive_window);
        f.quic_receive_window = self.quic_receive_window.unwrap_or(f.quic_receive_window);
...
async fn run_main(cli: Cli) -> anyhow::Result<()> {
    ...
    for (config_file, source) in config_files {
        ...
        log::info!(
            "\
            Starting lowertier from config file {:?}({:?}) with config:\n\
            ############### TOML ###############\n\
            {}\n\
            -----------------------------------\n\
            ",
    ...
    if crate_cli_network {
        ...
        log::info!(
            "\
            Starting lowertier from cli with config:\n\
            ############### TOML ###############\n\
            {}\n\
            -----------------------------------\n\
            ",
...
pub async fn main() -> ExitCode {
    ...
    if let Some(shell) = cli.gen_autocomplete {
        ...
        if let Some(shell) = shell.to_shell() {
            crate::print_completions(shell, &mut cmd, "lowertier-core");
        } else {
            // Handle Nushell
            crate::print_nushell_completions(&mut cmd, "lowertier-core");
        }
...
"#,
    ...
    #[test]
    fn cli_round_trips_strict_underlay_and_quic_options() {
        let cli = Cli::try_parse_from([
            "lowertier-core",
            "--underlay-deny-interfaces",
            "utun5,tailscale0",
            "--underlay-deny-cidrs",
            "100.64.0.0/10,fd7a:115c:a1e0::/48",
            "--quic-congestion",
            "brutal",
            "--quic-brutal-send-bps",
            "100000000",
            "--quic-brutal-loss-compensation=false",
            "--quic-initial-receive-window",
            "8388608",
            "--quic-receive-window",
            "33554432",
        ...
        assert_eq!(flags.underlay_deny_interfaces, ["utun5", "tailscale0"]);
        assert_eq!(
            flags.underlay_deny_cidrs,
            ["100.64.0.0/10", "fd7a:115c:a1e0::/48"]
        );
        assert_eq!(flags.quic_congestion, "brutal");
        assert_eq!(flags.quic_brutal_send_bps, 100_000_000);
    ...
    fn tun_cli_sets_device_name() {
        let cli = Cli::try_parse_from(["lowertier-core", "--tun=lower0"]).unwrap();
        let cfg = TomlConfigLoader::default();
    ...
    fn tun_cli_rejects_conflicting_device_name() {
        let cli =
            Cli::try_parse_from(["lowertier-core", "--tun=lower0", "--dev-name=lower1"]).unwrap();
        let cfg = TomlConfigLoader::default();
