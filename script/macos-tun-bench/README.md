...
This harness runs `easytier-core` natively through macOS utun and connects it to a Linux EasyTier
peer in Colima. It measures L2-TUN in both directions with parallel TCP streams, unloaded and
loaded latency, and native EasyTier CPU per Gbit.
...
- a release `easytier-core` binary built with the `tun` feature;
- passwordless `sudo` for launching and stopping the native client;
- a running Colima profile and an EasyTier benchmark image containing iperf3;
- host `iperf3` and `jq`.
...
not depend on EasyTier's computed default.
...
| `COLIMA_PROFILE` | `easytier-l2` | Peer VM profile |
| `DOCKER_CONTEXT` | `colima-easytier-l2` | Peer Docker endpoint |
...
loaded-latency samples, native `top` samples, RSS and thread samples, EasyTier CPU per Gbit, and
